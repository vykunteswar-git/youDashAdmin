...
import { Trash2, Package, Truck, Weight } from "lucide-react";
  ...
  return (
    <section className="surface p-5">
      <div className="flex items-start justify-between mb-4">
        ...
        <button type="button" onClick={addSlab} className="chip flex-shrink-0">
          + Add slab
        </button>
      </div>
      ...
      <div className="space-y-2">
        {slabs.map((slab, index) => (
          <div key={index} className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-2 items-end rounded-sm border border-[var(--border-default)] p-3">
            ...
            <button
              type="button"
              onClick={() => setSlab(index, "isActive", !slab.isActive)}
  ...
  return (
    <section className="surface p-5">
      <div className="flex items-start justify-between mb-1">
        ...
        <button type="button" onClick={addTier} className="chip flex-shrink-0">
          + Add tier
        </button>
      </div>
      ...
      <div className="space-y-3">
        {tiers.map((tier, index) => (
          <div
            ...
            className={`rounded-sm border p-4 space-y-3 ${
              !tier.vehicleId && tier.isActive !== false ? "border-rose-300 bg-rose-50/30" : "border-[var(--border-default)]"
            }`}
          >
            ...
            <div className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-3 items-end">
              ...
              <button
                type="button"
                onClick={() => setTier(index, "isActive", !tier.isActive)}
            ...
            {tier.vehicleId && (
              <div className="text-[11px] text-zinc-400 mono bg-zinc-50 rounded-sm px-3 py-2 border border-[var(--border-default)]">
                {Number(tier.minimumKm) > 0 ? (
                  <>
                    {tier.minWeightKg}–{tier.maxWeightKg} kg · &lt;{tier.minimumKm} km → ₹{tier.baseFare} flat · ≥{tier.minimumKm} km → ₹
                    {tier.baseFare} + dist × ₹{tier.ratePerKm}/km
                  </>
                ) : (
                  <>
        ...
        {tiers.length === 0 && <div className="empty">No tiers configured — fallback rate will apply to all parcels.</div>}
      </div>
    </section>
  );
}
